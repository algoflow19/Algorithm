...
class PriorityQueue:
    ...
    def fix_insert_violant(self,current_index):
        if(current_index<=1): return
        new_elem=self.queue[current_index]
        origin_elem=self.queue[current_index//2]
        if(new_elem<origin_elem):
            self.queue[current_index]=origin_elem
            self.queue[current_index//2]=new_elem
            self.fix_insert_violant(current_index//2)
    
    
    def fix_pop_violant(self,elem_pos):
        """
            First call the method when the violant is on the top of the heap.
        """
        if(elem_pos*2>len(self.queue)-1): return
        
        new_elem=self.queue[elem_pos]
        min_elem=None
        left=True
        if(elem_pos*2+1>len(self.queue)-1):
            min_elem=self.queue[elem_pos*2]
        else:
            if(self.queue[elem_pos*2] > self.queue[elem_pos*2+1]):
                min_elem=self.queue[elem_pos*2+1]
                left=False
            else:
                min_elem=self.queue[elem_pos*2]
        
        if(new_elem<min_elem): return
        
        if(left):
            self.queue[elem_pos]=min_elem
            self.queue[elem_pos*2]=new_elem
            self.fix_pop_violant(elem_pos*2)
        else:
            self.queue[elem_pos]=min_elem
            self.queue[elem_pos*2+1]=new_elem
            self.fix_pop_violant(elem_pos*2+1)
    ...
    def append(self, key):
        ...
        elem_pos=len(self.queue)-1
        self.fix_insert_violant(elem_pos)
    ...
    def pop(self):
        ...
        if(length<=1):
            raise UserWarning("No element in the queue!")
        if(length==2):
            return self.queue.pop()
        min=self.queue[1]
        last=self.queue.pop()
        self.queue[1]=last
        self.fix_pop_violant(1)
        return min
