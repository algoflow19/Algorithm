...
Using the recursion-tree method, we know that the depth of the tree is n, and the nodes in the tree at level i, for $1\leq i\leq n$, is $3\times 4^i$. We know that it's cost O(1) time for each node at level zero to n minus 1 and there is a important obversion that for level 1 to n, each node receive a $\frac{1}{3}$ length edge compare to its last level, so the triangle each node produced is $\frac{1}{9}$ area compared to its last level. Therefore, we know that for each level i range(1,n) , it grow $O((\frac{4}{9})^i)$ area at each leval, so the over all total asymptotic cost of rendering a snowflake with LoD n is $ c_1 \theta(4^n)+c_2\theta(\frac{4}{9}+(\frac{4}{9})^2+...+(\frac{4}{9})^n)=c_1 \theta(4^n)+c_2\theta(1)=c_1 \theta(4^n)$.
%%% PROBLEM 1(u) ANSWER END %%% 
}
\newcommand{\answerIIa}{ \answer 
%%% PROBLEM 2(a) ANSWER START %%%
PriorityQueue.\_find\_min
