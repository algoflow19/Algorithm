...
class BigNum(object):
  ...
  def __init__(self, digits, size = None, no_copy = False):
    ...
    self.byte_two=(Word.from_byte(Byte.one())+Word.from_byte(Byte.one())).lsb()
  @staticmethod
  def zero(size = 1):
  ...
  def slow_mul(self, other):
    ...
    return ((self_low*other_low)+((self_high*other_low+self_low*other_high) <<split) \
            + ((self_high*other_high)<<(2*split)) ).normalize()
  ...
  def __divmod__(self, other):
    ...
    self.normalize()
    other.normalize()
    if len(self.d) <= 128 or len(other.d) <= 128:
  ...
  def slow_divmod(self, other):
    ...
    '''
    byte_1=Byte.one()
    copy_self=BigNum(self.d,None,False).normalize()
    Si=BigNum(other.d,None,False).normalize()
    max_size=max(len(copy_self.d),len(Si.d))
    Q=BigNum.zero()
    
    i=(max_size-len(Si.d))*8
    Si=Si<<max_size-len(Si.d)
    if(Si<self): 
      Si=Si<<1
      i+=8
#    print(str(i))
...
#      print(Q)
      Q=Q.bit_lshift()
      if(not (copy_self<Si)):
        copy_self=copy_self-Si
        Q+=BigNum([byte_1],None,False)
        Si.normalize()
        copy_self.normalize()
      Si.bit_rshift()
    return (Q.normalize(),copy_self.normalize())
  ...
  def bit_lshift(self):
    l=len(self.d)
    current_word=Word.from_byte(self.d[l-1])
    current_word=current_word+current_word
    if(not (current_word.msb() == Byte.zero())):
      self=self+ (BigNum.one() << l)
#      self.d.append(Byte.one)
    for i in range(l-1,0,-1):
      current_word=Word.from_bytes(self.d[i],self.d[i-1])
      current_word+=current_word
      self.d[i]=current_word.msb()
    self.d[0]=(Word.from_byte(self.d[0])+Word.from_byte(self.d[0])).lsb()
    return self
    
  def bit_rshift(self):
    l=len(self.d)
    for i in range(0,l-1):
      current_word=Word.from_bytes(self.d[i+1],self.d[i])
      self.d[i]=current_word // self.byte_two
    self.d[l-1]=(Word.from_byte(self.d[l-1]) // self.byte_two)
