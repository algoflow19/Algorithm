...
#                 id(self.node) >= id(other.node)))
#    
Node.__lt__=lambda this,other:(this.distance < other.distance or 
                               (this.distance ==other.distance and
                                id(this) < id(other)))
Node.__le__=lambda this,other:(this.distance < other.distance or 
                               (this.distance ==other.distance and
                                id(this) <= id(other)))
Node.__gt__=lambda this,other:(this.distance > other.distance or 
                               (this.distance ==other.distance and
                                id(this) > id(other)))
Node.__ge__=lambda this,other:(this.distance > other.distance or 
                               (this.distance ==other.distance and
                                id(this) >= id(other)))
...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        for node in nodes:
          ...
          if(node==source):
            node.distance=0
          toDoNodes.insert(node)
#        print(source)
#        print(destination)
        while(True):
          visited_num+=1
          min_node=toDoNodes.extract_min()
#          print(min_node)
          if(min_node==None):
            return (None,visited_num)
...
#              print(node)
          ...
          for neighboor in min_node.adj:
            dis=min_node.distance+weight(min_node,neighboor)
            if(neighboor.distance > dis):
              neighboor.distance=dis
              neighboor.parent=min_node
              toDoNodes.decrease_key(neighboor)
