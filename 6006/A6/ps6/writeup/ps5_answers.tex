...
\begin{codebox}
\Procname{$\proc{MAX-EDGERANKS}(s,G,K)$}
...
\problem \points{30} \textbf{RenBook Competitor}
...
\begin{codebox}
\Procname{$\proc{DFS-VISITED}(n,G,F,P,s,end)$}
\li \If \proc{ISINSTALLED}(n) OR $n$ IN $F$\Do
\li \Return FALSE \End
...
\li \If \proc{DFS-VISITED}(n,G,F,P,s,end) $==$TURE
...
\begin{codebox}
\Procname{$\proc{INSTALL-ORDER-PRODUCER}(t,G)$} 
\li INITIALIZZE-SINGLE-SOURCE(G,s)
\li $front \gets [s]$
\li $tmp \gets [$ $]$
\li \For $i \gets 1$ \To $K$ \Do
