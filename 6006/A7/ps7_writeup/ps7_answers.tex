...
There are four investment way, Which are:
%(1) buy one stock of Dale, Inc. we revecive $8+39=47$.
%(2) buy two stocks of JCN, Corp. we revecive $26$.
%(3) buy one stock of Dale, Inc. we revecive $2+47=49$.
%(4) buy one stock of Dale, Inc. we revecive $5+45=50$.
%So we choose the fourth plan.
0
...
\li $current\_total \gets total$
\li $key=\proc{hash}(\total,limit)$) 
\li \If $key$ In $memo$ \Then
\li \Return memo[key] \End
\li \For $i \gets 1$ \To $stockCount$ \Then
\li \If $limit[i]>=1$ And $priceStart[i] <= total$ \Then
\li $limit[i] \gets limit[i]-1$
\li $tmp\gets \proc{StockLimited}(\total-priceStart[i], \stockCount, \priceStart, \priceEnd, \limit) + priceEnd[i]$
\li $limit[i]\gets limit[i] +1$
\li \Else Continue \End 
\li \If $tmp>=current\_total$ \Then
\li $current\_total \gets tmp$
\li $memo[key]=current\_total$ \End \End
\li \Return $current\_total$
\end{codebox}
First, using the dynamic programming, we figure out how many subproblems, and the cost of each of them, then we get the answer(No merge required in this problem). The subproblem is identified by total and limit, So we know there are $total \times 2^{\proc{sum}(\limit)}$ of them. Notice that the sum ignore the infinite elements, and from the code we know that each subproblem cost constant time, so we know the runtime of the algothrims is 
$total \times 2^{\proc{sum}(\limit)}$.

The recurrence relation of this algothrims is:

memo[(total,limit)]=
max\{
memo[(total-startPrice[i],limit) 

\quad \quad \quad for i=1 To stockCount,
If limit[i]$>$0, limit[i]=limit[i]-1
\}
